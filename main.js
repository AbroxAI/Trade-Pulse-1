(function() {
  ...
  function updateAll() {
    ...
    // Keep totalProfit static (set at the top)
  ...
  function setWithdrawMethod(method) {
    if (method === activeWithdrawMethod && withdrawFormContainer.dataset.method === method) {
      return;
    }
  ...
  function getAvatarTextNode() {
    return Array.from(avatarContainer.childNodes).find(
      (node) => node.nodeType === Node.TEXT_NODE && node.textContent.trim() === 'T'
    );
  }
  ...
  function renderOverviewCards() {
    ...
    let totalProfitValue = base.totalProfit,
        investedValue = base.totalInvested,
        referralValue = 12000,
        withdrawnValue = base.totalWithdrawn;
    ...
    if (overviewPeriod === 'week') {
      ...
      totalProfitValue = base.totalProfit;
      investedValue = base.totalInvested;
      referralValue = 12000;
      withdrawnValue = base.totalWithdrawn;
    }
    ...
    let cards = [
      ...
      { title:"Active Investments", value:investedValue, sub:`${base.activePlans} Plan${base.activePlans!==1?'s':''}`, subColor:"#94a3b8", iconBg:"rgba(59,130,246,.12)", iconColor:"#60a5fa", icon:'<path d="M16 11c1.7 0 3-1.3 3-3s-1.3-3-3-3-3 1.3-3 3 1.3 3 3 3Z"/><path d="M8 13c1.7 0 3-1.3 3-3S9.7 7 8 7 5 8.3 5 10s1.3 3 3 3Z"/>' },
    ...
    c.innerHTML = cards.map(ca => `
      <div class="stat" data-title="${ca.title}" data-value="${fmt(ca.value)}" data-sub="${ca.sub}">
        ...
        <div class="value">${balanceHidden?'****':fmt(ca.value)}</div>
  ...
  document.getElementById('finalConfirmWithdrawBtn').addEventListener('click', () => {
    ...
    setTimeout(() => {
      ...
      // update profit appropriately: profit = profit - amount? Not realistic, so we keep profit static; but if you need dynamic, uncomment next line
      // base.totalProfit = base.homeBalance + base.totalWithdrawn - base.totalInvested;
      chartData.push({ label: new Date().getHours()+':'+String(new Date().getMinutes()).padStart(2,'0'), value: base.homeBalance });
