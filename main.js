(function() {
  ...
  document.querySelectorAll('.method-card').forEach(c => {
    c.addEventListener('click', () => {
      ...
      // FIX: update balances without glitch
      if (activeWithdrawMethod === 'usdt_trc20' || activeWithdrawMethod === 'usdt_bep20') {
        const usdtBalance = 1000.00;
        document.getElementById('withdrawBalance').textContent = `${usdtBalance.toFixed(2)} USDT`;
        document.getElementById('withdrawFx').textContent = `≈ $ ${(usdtBalance * rates['USD']).toFixed(2)}`;
        document.getElementById('withdrawableAmount').textContent = `${(usdtBalance-1).toFixed(2)} USDT`;
      } else {
        updateAll();
      }
    });
  ...
  // Drawer actions (avatar listener removed)
  ...
  // Logout is handled in index.html – do not add handler here
