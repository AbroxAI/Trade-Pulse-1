(function() {
  ...
  // ========== BANK ICON MAP ==========
  const bankIconMap = {
    'GTBank': 'assets/banks/gtbank.png',
    'Opay': 'assets/banks/opay.png',
    'Palmpay': 'assets/banks/palmpay.png',
    'Access Bank': 'assets/banks/access.png',
    'First Bank': 'assets/banks/firstbank.png',
    'UBA': 'assets/banks/uba.png',
    'Zenith Bank': 'assets/banks/zenith.png',
    'Kuda Bank': 'assets/banks/kuda.png',
    'Wema Bank': 'assets/banks/wema.png',
    'USDT': 'assets/banks/usdt.png',
    'Wallet': 'assets/banks/wallet.png'  // generic wallet
  };

  function extractBankName(subtitle) {
    if (!subtitle) return '';
    // Handle "To X" or "From X"
    let name = subtitle.replace(/^(To|From)\s+/i, '').trim();
    // If it contains USDT, return USDT
    if (name.toLowerCase().includes('usdt')) return 'USDT';
    // If it's a bank name, return as is (may have extra text after hyphen, ignore)
    // For simplicity, we take the whole string; our map keys include exact names
    return name;
  }

  function getBankIconPath(bankName) {
    if (bankIconMap[bankName]) return bankIconMap[bankName];
    // Try to match partial (e.g., "First Bank" might be only "First")
  ...
  function getTxIconHTML(tx) {
    // Use the subtitle to determine the bank/wallet
    let bankName = extractBankName(tx.subtitle);
    let iconPath = getBankIconPath(bankName);
    if (iconPath) {
      // Use an <img> with onerror fallback to the default bank SVG
      return `<div class="tx-ico-img" style="width:40px;height:40px;border-radius:50%;overflow:hidden;display:grid;place-items:center;"><img src="${iconPath}" style="width:100%;height:100%;object-fit:cover;" onerror="this.style.display='none';this.parentNode.innerHTML='${fallbackBankSVG.replace(/'/g, "\\'")}';"></div>`;
    } else {
      // Fallback: show the generic bank SVG (same as old bank icon)
      return `<div class="tx-ico" style="background:${tx.amount>0?'rgba(34,197,94,.12)':'rgba(245,158,11,.12)'}">${fallbackBankSVG}</div>`;
    }
  }

  // ★ UPDATED BASE – balance 621,435, plan min 10,000
  const base = {
    homeBalance: 621435,
    totalInvested: 280000,
    totalProfit: 220000,
    totalWithdrawn: 543500,
    walletTotal: 621435,
    walletAvailable: 617435,
    walletLocked: 4000,
    nairaWallet: 617435,
    withdrawable: 617435,
    txFee: FEE,
  ...
  let chartData = [
    ...
    { label:'Mar 01', value:18000 },
    { label:'Mar 10', value:42000 },
    { label:'Mar 20', value:72000 },
    { label:'Mar 30', value:115000 },
    { label:'Apr 05', value:162000 },
    { label:'Apr 12', value:222000 },
    { label:'Apr 20', value:291000 },
    { label:'Apr 28', value:355000 },
    { label:'May 05', value:419000 },
    { label:'May 10', value:488000 },
    { label:'May 15', value:549000 },
    { label:'May 18', value:592000 },
    { label:'May 22', value:621435 }
  ];

  let allTransactions = [
    { title:'Deposit', subtitle:'From GTBank', meta:'Feb 10, 2026 09:15 AM', amount:150000, amountColor:'#4ade80', iconType:'bank', status:'Completed' },
    { title:'Deposit', subtitle:'From GTBank', meta:'Feb 12, 2026 11:20 AM', amount:200000, amountColor:'#4ade80', iconType:'bank', status:'Completed' },
    { title:'Deposit', subtitle:'From Palmpay', meta:'Feb 14, 2026 03:45 PM', amount:350000, amountColor:'#4ade80', iconType:'opay', status:'Completed' },
    { title:'Withdrawal', subtitle:'To Access Bank', meta:'Feb 16, 2026 10:30 AM', amount:-25000, amountColor:'#f87171', iconType:'bank', status:'Completed' },
    { title:'Withdrawal', subtitle:'To Opay', meta:'Feb 20, 2026 02:10 PM', amount:-18000, amountColor:'#f87171', iconType:'opay', status:'Completed' },
    { title:'Deposit', subtitle:'From GTBank', meta:'Feb 25, 2026 01:00 PM', amount:120000, amountColor:'#4ade80', iconType:'bank', status:'Completed' },
    { title:'Withdrawal', subtitle:'To GTBank', meta:'Mar 01, 2026 09:05 AM', amount:-32000, amountColor:'#f87171', iconType:'bank', status:'Completed' },
    { title:'Withdrawal', subtitle:'To First Bank', meta:'Mar 08, 2026 04:20 PM', amount:-22000, amountColor:'#f87171', iconType:'bank', status:'Completed' },
    { title:'Deposit', subtitle:'From Palmpay', meta:'Mar 15, 2026 07:30 AM', amount:80000, amountColor:'#4ade80', iconType:'opay', status:'Completed' },
    { title:'Withdrawal', subtitle:'To UBA', meta:'Mar 22, 2026 11:55 AM', amount:-45500, amountColor:'#f87171', iconType:'bank', status:'Completed' },
    { title:'Withdrawal', subtitle:'To Zenith Bank', meta:'Mar 30, 2026 08:15 AM', amount:-30000, amountColor:'#f87171', iconType:'bank', status:'Completed' },
    { title:'Deposit', subtitle:'From GTBank', meta:'Apr 05, 2026 10:45 AM', amount:95000, amountColor:'#4ade80', iconType:'bank', status:'Completed' },
    { title:'Withdrawal', subtitle:'To Palmpay', meta:'Apr 10, 2026 05:10 PM', amount:-36500, amountColor:'#f87171', iconType:'opay', status:'Completed' },
    { title:'Withdrawal', subtitle:'To GTBank', meta:'Apr 18, 2026 12:20 PM', amount:-52000, amountColor:'#f87171', iconType:'bank', status:'Completed' },
    { title:'Deposit', subtitle:'From Binance (USDT)', meta:'Apr 25, 2026 02:30 PM', amount:750, amountColor:'#4ade80', currency:'USD', iconType:'usdt', status:'Completed' },
    { title:'Withdrawal', subtitle:'To Opay', meta:'May 02, 2026 09:40 AM', amount:-41000, amountColor:'#f87171', iconType:'opay', status:'Completed' },
    { title:'Withdrawal', subtitle:'To First Bank', meta:'May 10, 2026 03:15 PM', amount:-38500, amountColor:'#f87171', iconType:'bank', status:'Completed' },
    { title:'Deposit', subtitle:'From Palmpay', meta:'May 15, 2026 06:05 AM', amount:65000, amountColor:'#4ade80', iconType:'opay', status:'Completed' },
    { title:'Withdrawal', subtitle:'To Access Bank', meta:'May 22, 2026 07:50 AM', amount:-29000, amountColor:'#f87171', iconType:'bank', status:'Completed' },
    { title:'Withdrawal', subtitle:'To UBA', meta:'Jun 01, 2026 01:25 PM', amount:-46000, amountColor:'#f87171', iconType:'bank', status:'Completed' },
    { title:'Deposit', subtitle:'From GTBank', meta:'Jun 08, 2026 11:00 AM', amount:104935, amountColor:'#4ade80', iconType:'bank', status:'Completed' },
    { title:'Withdrawal', subtitle:'To Palmpay', meta:'Jun 12, 2026 04:35 PM', amount:-35000, amountColor:'#f87171', iconType:'opay', status:'Completed' },
    { title:'Withdrawal (USDT)', subtitle:'To Wallet', meta:'Jun 16, 2026 09:20 AM', amount:-200, amountColor:'#f87171', currency:'USD', iconType:'usdt', status:'Completed' }
  ];
  ...
  function setWithdrawMethod(method) {
    if (method === activeWithdrawMethod && withdrawFormContainer.dataset.method === method) return;
    activeWithdrawMethod = method;
    withdrawFormContainer.dataset.method = method;
  ...
  function renderWithdrawForm(method) {
    let html = '';
    if (method === 'bank') {
      html = `<div class="field-label">Recipient Account Number</div>
      ...
      html = `<div class="field-label">Wallet Address (${network})</div>
      ...
      html = `<div class="field-label">E-Wallet ID / Email</div>
  ...
  function getAvatarTextNode() {
    return Array.from(avatarContainer.childNodes).find(node => node.nodeType === Node.TEXT_NODE && node.textContent.trim() === 'T');
  }
  function updateAvatarDisplay(dataUrl) {
    ...
    const textNode = getAvatarTextNode();
  ...
  else resetDefaultAvatar();

  if (cameraIcon) cameraIcon.addEventListener('click', e => { e.stopPropagation(); avatarInput.click(); });
  else if (avatarContainer) avatarContainer.addEventListener('click', () => avatarInput.click());
  ...
  if (avatarInput) {
    ...
    newAvatarInput.addEventListener('change', function(e) {
      ...
      reader.onload = function(ev) { updateAvatarDisplay(ev.target.result); };
  ...
  if (logoutBtn) {
    ...
    newLogoutBtn.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); logoutModal.classList.add('open'); });
  }
  if (confirmLogoutBtn) confirmLogoutBtn.addEventListener('click', () => { sessionStorage.clear(); localStorage.clear(); location.reload(); });
  if (cancelLogoutBtn) cancelLogoutBtn.addEventListener('click', () => logoutModal.classList.remove('open'));
  if (logoutModal) logoutModal.addEventListener('click', e => { if (e.target === logoutModal) logoutModal.classList.remove('open'); });
  ...
  document.querySelectorAll('.invest-now-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const rate = plan==='beginner'?3.0:plan==='standard'?3.5:plan==='premium'?4.0:4.5;
  ...
  function showModal(t, arr) {
    ...
    arr.forEach(tx => {
      ...
      row.innerHTML = `${getTxIconHTML(tx)}<div class="tx-main"><div class="tx-title">${tx.title}</div><div class="tx-sub">${tx.subtitle} • ${tx.meta}</div></div><div class="tx-right"><div class="tx-amt" style="color:${tx.amountColor}">${s}</div><div class="status">${tx.status}</div></div>`;
  ...
  function renderRecentTx() {
    ...
    allTransactions.slice(-4).reverse().forEach(tx=>{
      let s=tx.currency==='USD'? `${tx.amount>=0?'+':'-'}$${Math.abs(tx.amount).toFixed(2)}`: `${tx.amount>=0?'+':''}${fmt(Math.abs(tx.amount))}`;
      c.innerHTML+=`<div class="tx-row" data-tx-id="${tx.id}">${getTxIconHTML(tx)}<div class="tx-main"><div class="tx-title">${tx.title}</div><div class="tx-sub">${tx.subtitle} • ${tx.meta}</div></div><div class="tx-right"><div class="tx-amt" style="color:${tx.amountColor}">${s}</div><div class="status">${tx.status}</div></div></div>`;
    });
  }
  ...
  function renderWalletTx() {
    ...
    allTransactions.slice(-4).reverse().forEach(tx=>{
      let s=tx.currency==='USD'? `${tx.amount>=0?'+':'-'}$${Math.abs(tx.amount).toFixed(2)}`: `${tx.amount>=0?'+':''}${fmt(Math.abs(tx.amount))}`;
      c.innerHTML+=`<div class="wallet-tx-row">${getTxIconHTML(tx)}<div class="tx-main"><div class="tx-title">${tx.title}</div><div class="tx-sub">${tx.subtitle} • ${tx.meta}</div></div><div class="tx-right"><div class="tx-amt" style="color:${tx.amountColor}">${s}</div><div class="status">${tx.status}</div></div></div>`;
    });
  }
  function renderOverviewCards() {
    ...
    if (overviewPeriod === 'week') { totalProfitValue = Math.round(base.totalProfit * 0.25); investedValue = Math.round(base.totalInvested * 0.25); referralValue = 3000; withdrawnValue = Math.round(base.totalWithdrawn * 0.2); }
    else if (overviewPeriod === 'month') { totalProfitValue = base.totalProfit; investedValue = base.totalInvested; referralValue = 12000; withdrawnValue = base.totalWithdrawn; }
    let cards = [
  ...
  // ========== WITHDRAWAL RECEIPT MODAL WITH ICON ==========
  function openWithdrawalReceipt(tx) {
    ...
    // Insert bank icon into receipt
    const receiptBankSpan = document.getElementById('receiptBank');
    const iconPath = getBankIconPath(bankName);
    if (iconPath) {
      receiptBankSpan.innerHTML = `<img src="${iconPath}" style="width:24px;height:24px;border-radius:4px;vertical-align:middle;margin-right:6px;" onerror="this.style.display='none';"> ${bankName}`;
    } else {
      receiptBankSpan.textContent = bankName;
    }
  ...
  document.getElementById('withdrawalReceiptModal').addEventListener('click', (e) => {
    if (e.target === document.getElementById('withdrawalReceiptModal')) document.getElementById('withdrawalReceiptModal').classList.remove('open');
  });
  ...
  (function updateDrawerName() {
    const savedUser = sessionStorage.getItem('tradePulseCurrentUser');
    if (savedUser) {
      try {
        const user = JSON.parse(savedUser);
        const nameEl = document.getElementById('drawerFullName');
