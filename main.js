(function() {
  ...
  // FIXED: withdraw method switch – no flicker, update balances smoothly
  document.querySelectorAll('.method-card').forEach(c => {
    c.addEventListener('click', () => {
      ...
      if (activeWithdrawMethod === 'usdt_trc20' || activeWithdrawMethod === 'usdt_bep20') {
        ...
        if (withdrawBalanceEl) withdrawBalanceEl.textContent = `${usdtBalance.toFixed(2)} USDT`;
        if (withdrawFxEl) withdrawFxEl.textContent = `≈ $ ${(usdtBalance * rates['USD']).toFixed(2)}`;
        if (withdrawableEl) withdrawableEl.textContent = `${(usdtBalance-1).toFixed(2)} USDT`;
      } else if (activeWithdrawMethod === 'ewallet') {
  ...
  // Removed avatar-related listeners (handled in index.html)
  // Removed logout handler (handled in index.html)
